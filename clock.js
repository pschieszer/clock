...
const addFilter = (srcFilter, filtColor) => {
  const newFilter = srcFilter.cloneNode(true);
  newFilter.id = `textShadow-${filtColor}`;
  srcFilter.parentNode.appendChild(newFilter);
  [...newFilter.children].filter(x => x.nodeName == 'feFlood')[0].setAttribute('flood-color', `hsl(${filtColor} 100% 50%)`);
};
...
result.setAttribute("id", id);
  result.setAttribute("font-family", "Consolas");
const currCol = `hsl(${color} 100% 50%)`;
  const filtColor = (color + 180) % 360;
  if (filtColor) addFilter(document.querySelector('filter#textShadow-0'), filtColor);
result.setAttribute("fill", currCol);
result.setAttribute("stroke", currCol);
  result.setAttribute("filter", `url(#textShadow-${filtColor})`);
...
const updateHands = () => {
  ...
  document.getElementById('hourHand').setAttribute('stroke', `hsl(${hourOffset * 6} 100% 30%)`);
  ...
  document.getElementById('minuteHand').setAttribute('stroke', `hsl(${minuteColor} 100% 70%)`);
